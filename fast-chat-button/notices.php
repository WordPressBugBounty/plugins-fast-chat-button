...
// ============================================================
// replace the prefix when adding to new plugin "yydev_chat_btn_"
// ============================================================

$yydev_chat_btn_notice_info_array = array(
	'plugin_name' => "Fast Chat Button", // the name of the plugin
	'developer_website' => "https://www.yydevelopment.com", // link to the developer website page
	'plugin_review_page' => "https://wordpress.org/plugins/fast-chat-button/#reviews", // link to the plugin support page 
	'plugin_support_link' => "https://wordpress.org/support/plugin/fast-chat-button/", // link to the plugin review page 
	'plugin_donate_link' => "https://www.yydevelopment.com/coffee-break/?plugin=fast-chat-button", // link to the plugin donate page
	...
	'save_database_time_stamp_name' => "yydev_chat_btn_timestamp",	// database input name to save data 
...
$plugin_db_timestamp = get_option($yydev_chat_btn_notice_info_array['save_database_time_stamp_name']);
...
if( empty($plugin_db_timestamp) ) {
	...
	update_option($yydev_chat_btn_notice_info_array['save_database_time_stamp_name'], $yydev_chat_btn_notice_info_array['send_mail_in_days']);
	$plugin_db_timestamp = get_option($yydev_chat_btn_notice_info_array['save_database_time_stamp_name']);
...
function yydev_chat_btn_stop_notice_forever() {

	// Verify nonce for security
	if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'yydev_chat_btn_nonce')) {
	...
	global $yydev_chat_btn_notice_info_array;
	update_option($yydev_chat_btn_notice_info_array['save_database_time_stamp_name'], 'stop');
	wp_die(); // Use wp_die() instead of die() for WordPress AJAX
	
} // function yydev_chat_btn_stop_notice_forever() {

add_action( 'wp_ajax_yydev_chat_btn_stop_notice_forever', 'yydev_chat_btn_stop_notice_forever' );


// when the visitor ask to get the message in the future
function yydev_chat_btn_stop_notice_for_now() {
...
add_action( 'wp_ajax_yydev_chat_btn_stop_notice_for_now', 'yydev_chat_btn_stop_notice_for_now' );
...
function yydev_chat_btn_admin_notice($notice_info_array) {

	global $yydev_chat_btn_notice_info_array;

	$plugin_name = $yydev_chat_btn_notice_info_array['plugin_name'];
	$developer_website = $yydev_chat_btn_notice_info_array['developer_website'];
	$plugin_review_page = $yydev_chat_btn_notice_info_array['plugin_review_page'];
	$plugin_support_link = $yydev_chat_btn_notice_info_array['plugin_support_link'];
	$plugin_donate_link = $yydev_chat_btn_notice_info_array['plugin_donate_link'];
	$company_plugins_page = $yydev_chat_btn_notice_info_array['company_plugins_page'];
	$icon_image_path = $yydev_chat_btn_notice_info_array['icon_image_path'];
	$save_database_time_stamp_name = $yydev_chat_btn_notice_info_array['save_database_time_stamp_name'];
...
<script>
	...
	jQuery(document).ready(function($){
		...
		$(document).on('click', '.yydev_chat_btn_notice_style .yy-plugin-dismiss-forever', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {
				...
				var data = {
					'action': 'yydev_chat_btn_stop_notice_forever',
					'nonce': '<?php echo wp_create_nonce('yydev_chat_btn_nonce'); ?>'
				};
				jQuery.post(ajaxurl, data,  function(response) {});

			}); // $( relatedPostBox ).animate({opacity: "0"}, 1000, function() {
			return false;
		}); // $(document).on('click', '.yydev_chat_btn_notice_style notice-dismiss', function() {

		// if the user click on ask me later
		$(document).on('click', '.yydev_chat_btn_notice_style .yy-plugin-dismiss-for-now', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {
				...
				// use the function and update value using ajax
				var data = {
					'action': 'yydev_chat_btn_stop_notice_for_now',
					'nonce': '<?php echo wp_create_nonce('yydev_chat_btn_nonce'); ?>'
				};
				jQuery.post(ajaxurl, data,  function(response) {});
		...
		// if the user click on the right close button or ask to stop showing message
		$(document).on('click', '.yydev_chat_btn_notice_style .yy-review-plugin', function() {
			$(this).parent().parent().animate({opacity: "0"}, 200, function() {
...
<style>
	...
	.yydev_chat_btn_notice_style .yy-bottom-plugin-name {
		color: #fff;
		padding: 3px 10px;
		position: absolute;
		bottom: -20px;
	...
	.yydev_chat_btn_notice_style .notice-buttons {
		margin: 10px 0px 10px 0px;
	}

	.yydev_chat_btn_notice_style .notice-buttons a {
		max-width: 100%;
		white-space: normal;
		margin: 4px 0px 4px 2px;
	}

	.yydev_chat_btn_notice_style .yy-notice-dismiss {
		position: absolute;
		top: 0;
		left: auto;
		right: 1px;
	...
	.yydev_chat_btn_notice_style .yy-notice-dismiss::before {
		background: 0 0;
		color: #72777c;
		content: "\f153";
		display: block;
	...
	.yydev_chat_btn_notice_style .yy-plugin-icon {
		max-width: 85px;
		float: left;
	...
	<div class="yydev_chat_btn_notice_style notice notice-info">
	...
	</div><!--yydev_chat_btn_notice_style-->

<?php

} // function yydev_chat_btn_admin_notice() {
...
if( !empty($plugin_db_timestamp) && ($plugin_db_timestamp != "stop") && ($plugin_db_timestamp < strtotime("now")) && ($yydev_stop_plugins_notice != 1) ) {
	...
	add_action( 'admin_notices', 'yydev_chat_btn_admin_notice' );

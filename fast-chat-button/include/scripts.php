...
jQuery("document").ready(function($) {
    ...
    // ==================================================
    // Changing the url when changing chat type
    // ==================================================

    $('.yydevelopment-btn-chat .chat_button_type').on('change', function() {
        
        var imgURL = $(this).val();
        if( imgURL === 'custom') {
            $(".yydev_upload_image").trigger("click");
        } else {
            $(".yydevelopment-btn-chat img.yydev_light_img_bg").attr("src", imgURL);
            $(".yydevelopment-btn-chat .yydev_image_input").val(imgURL);
        }
